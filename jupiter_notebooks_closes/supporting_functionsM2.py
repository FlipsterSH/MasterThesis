...
def calculate_sharpe_ratio(daily_returns, risk_free_rate=0.0, annualize=True):
    """
    Calculate the Sharpe Ratio for a given list of daily returns.
    
    :param daily_returns: List of daily returns in decimal form (e.g. 0.01 for 1%).
    :param risk_free_rate: Daily risk-free rate in decimal form (default=0.0).
    :param annualize: Whether to return the annualized Sharpe Ratio (default=True).
    :return: Sharpe Ratio (annualized if annualize=True, otherwise daily).
    """
    # Convert to a numpy array for easier math operations
    ...
    std_dev = np.std(returns_array, ddof=1)  # using sample std with ddof=1 is common in finance

    # Handle the edge case of zero (or near-zero) volatility
    if std_dev == 0:
        return 0.0  # or you might choose to return None or raise an exception
